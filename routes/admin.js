...
router.get('/properties', passport.authenticate('jwt', {
  session: false
}), (req, res, next) => {
...
router.patch('/ban', passport.authenticate('jwt', {
  session: false
}), (req, res, next) => {
  if (req.user.admin < 1) return res.status(403).send("You are not admin!");
...
router.patch('/admin', passport.authenticate('jwt', {
  session: false
}), (req, res, next) => {
  if (req.user.admin < 2) return res.status(403).send("You are not head admin!");
  User.updateOne(req.body, (err) => {
    if (err)
      return res.status(500).send("Server error!");
